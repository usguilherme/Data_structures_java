...
public class RecursiveSingleLinkedListMethodsImpl<T> implements LinkedList<T> {
    ...
    public void reverse() {
        if (!node.isEmpty() && !node.getNext().isEmpty()) {
            node = reverse(node, new RecursiveSingleLinkedListImpl<>()); // inicializa previous com NIL
        }
    }

    private RecursiveSingleLinkedListImpl<T> reverse(RecursiveSingleLinkedListImpl<T> current, RecursiveSingleLinkedListImpl<T> previous) {
        if (current.isEmpty()) {
            return previous; // quando chegar no NIL, previous será a nova cabeça
        }
        RecursiveSingleLinkedListImpl<T> next = current.getNext(); // guarda próximo
        current.setNext(previous); // inverte ponteiro
        return reverse(next, current); // recursão
    }
